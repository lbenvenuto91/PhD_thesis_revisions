...
#define AGSYNC1    78        /* gter message sync code 1 "N" */
#define AGSYNC2    66        /* gter message sync code 2 "B" */
...
{
	double freq;
	if (gnssdata.CarrierFrequencyHz == 0) 
	{
		freq = 154 * 10.24E6;
	}
	else {
...
double computePseudorange(androidgnssmeas gnssdata, float psdrgBias, int* gpsweek, double* gpssow)
{
	...
	*gpssow = local_est_GPS_time * NS_TO_S - *gpsweek * GPS_WEEKSEC;
	...
	/*LB: compute satellite emission time*/

	else {
		/*LB: split cases depending on different constellations*/
...
{
    /* LB: code to decode android message */

	const char delimiters[] = ",";
	char* running;
	char* currstrval; /*LB: current field read in the str*/
	int ncolmeas = 30; /*LB: number of fields for single measurement*/
	int nmeas;
	...
	/*LB: read the number of Raw meas sent for this epoch*/
	int i;
	for (i = 0; i < 2; i++) {
	...
	/*LB: read values in every Raw meas*/
	...
	for (n = 0; n < nmeas; n++) {
		...
		raw->obs.data[t].LLI[f] = ((andrawdata.ADRState > 2) ? 1 : 0); 
		raw->obs.data[t].code[f] = andcode2rtklibcode(code); 
